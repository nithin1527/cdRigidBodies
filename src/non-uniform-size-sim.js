...
export function initNonUniformSizeSim() {
   ...
   */
    ...
    */
    ...
    let {initVelocity, initAngularVelocity} = window.getUIState()['non-uniform-size-settings'];
    function spawnPolyhedraInBoundingCube(numObjs, minSize, maxSize, complexity) {
        ...
        for (let i = 0; i < numObjs; i++) {
            ...
            while (!valid && attempts > 0) {
                let polyObj = spawnPolyhedron(
                    currObjsInScene, 
                    getRandomNumberMinMax(minSize, maxSize), 
                    complexity,
                );
    ...
    */
    ...
    function respondToCollisionObj2Obj(obj1, obj2) {
        ...
        let depth = r1 + r2 - dist; // essentially how much one object penetrates the other, using sphere so its pretty simple!
    ...
    */
    ...
    function isSphereInCube(bs) {
        ...
        let r = bs.radius;
        let c = bs.center;
        return c.x - r > minx && c.y - r > miny && c.z - r > minz &&
               c.x + r < maxx && c.y + r < maxy && c.z + r < maxz;
    }
    ...
    */
    ...
    class BVHNode {
        ...
        mergeBoundingSpheres() { // merge the bounding spheres of the objects in the node
            if (this.objs.length == 1) {
                ...
                this.bs.radius = refBs.radius;
            } else {
                let bs1 = this.objs[0].boundingSphere;
                let bs2 = this.objs[1].boundingSphere;
                let c = centroid([bs1.center, bs2.center]);
                this.bs.center = c;
                this.bs.radius = Math.max(c.distanceTo(bs1.center) + bs1.radius,  c.distanceTo(bs2.center) + bs2.radius);
            }
        }
    }
    ...
    function getCenterAndRadius(node) {
        ...
        let r_left = leftBs.radius;
        let r_right = rightBs.radius;

        let c = centroid([c_left, c_right]);
        // r = max( ||c-c_left|| + r_left, ||c-c_right|| + r_right )
        let r = Math.max(c.distanceTo(c_left) + r_left,  c.distanceTo(c_right) + r_right); 
        return {c, r};
    }   
    ...
    function BVHDetectCollisionsHelperAcrossLeftRight(node_a, node_b) {
        ...
        if ((!node_a || !node_b) || !intersectsSphere(node_a.bs, node_b.bs)) return;
    ...
    */
    let isPlaying = false;
    let playBtn = document.getElementById('non-uniform-size-play-btn');
    ...
    let root;
    let addPolyObjsBtn = document.getElementById('non-uniform-size-add-poly-objs-btn') 
    addPolyObjsBtn.addEventListener('click', () => {
        let {numObjects, size_min, size_max, complexity} = window.getUIState()['non-uniform-size-settings'];
        spawnPolyhedraInBoundingCube(numObjects, size_min, size_max, complexity);
    ...
    // super cool!!
    let randomizeVelBtn = document.getElementById('non-uniform-size-randomize-vel-btn');
    ...
    const bvhBtn = document.querySelector('#non-uniform-size-bvh-btn');
    const bfBtn = document.querySelector('#non-uniform-size-brute-force-btn');
    const bvSphereBtn = document.querySelector('#non-uniform-size-sphere-btn');
    const bvAABBBtn = document.querySelector('#non-uniform-size-aabb-btn');
    
    bvhBtn.addEventListener('click', () => {
        bvhBtn.classList.add('non-uniform-active-algo-type');
        bfBtn.classList.remove('non-uniform-active-algo-type');
        bvAABBBtn.classList.remove('non-uniform-active-bv-type');
        bvSphereBtn.classList.add('non-uniform-active-bv-type');
    });

    bfBtn.addEventListener('click', () => {
        bfBtn.classList.add('non-uniform-active-algo-type');
        bvhBtn.classList.remove('non-uniform-active-algo-type');
    });
    ...
    bvAABBBtn.addEventListener('click', () => {
        bvAABBBtn.classList.add('non-uniform-active-bv-type');
        bvSphereBtn.classList.remove('non-uniform-active-bv-type');
        bfBtn.classList.add('non-uniform-active-algo-type');
        bvhBtn.classList.remove('non-uniform-active-algo-type');
    });
